...
def _colorbar(top):
    return dict(title=dict(text="Höhe", side="right"), orientation="h", thickness=9, len=0.6, x=0.5, xanchor="center", y=-0.04, yanchor="top", tickmode="linear", tick0=0, dtick=max(1, top // 8))
...
def build_state(net, frame, height=460):
    ...
    if active:
        fig.add_trace(go.Scatter(x=[net.pos[v][0] for v in active], y=[net.pos[v][1] for v in active], mode="markers+text", name="Knoten mit Überschuss", text=[f"+{frame.excess[v]}" for v in active], textposition="middle right",
                                 textfont=dict(color=C.COLORS["cut"], size=12), hoverinfo="skip", marker=dict(symbol="circle-open", size=[16 + min(14, 3 * frame.excess[v] ** 0.5) for v in active], color=C.COLORS["cut"], line=dict(width=2.5))))
...
def build_configs(rows, dinic_mean, ek_mean, height=340):
    ...
    fig.add_hline(y=dinic_mean, line=dict(color="#2ca02c", dash="dash"), annotation_text="Dinic", annotation_position="top left")
    fig.add_hline(y=ek_mean, line=dict(color="#1f77b4", dash="dash"), annotation_text="Edmonds-Karp", annotation_position="top left")
